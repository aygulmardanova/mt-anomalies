...
//        if (Math.abs(beta.get(j, 0)) < 1E-4) return 0.0;
    ...
    public void printPredictedResults(double[] x, double[] y) {

        System.out.println(" Input              Approximation");
        System.out.println(" time   coord       coord1");
        for (int i = 0; i < x.length; ++i) {
            System.out.printf("%2.1f %3.1f  %5.1f\n", x[i], y[i], predict(x[i]));
        }
    ...
    public static void main(String[] args) {
        ...
        regression.printPredictedResults(x, y);
    }
}
